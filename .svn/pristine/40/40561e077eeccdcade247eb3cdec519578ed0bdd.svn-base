 ...
 * 1) Attrs: {@link R.styleable#DiniFlyAnimationView_dinifly_fileName}
 * 2) Programatically: {@link #setAnimation(String)}, {@link #setComposition(LottieComposition)},
 * or {@link #setAnimation(JSONObject)}.
 * <p>
 * You can set a default cache strategy with {@link R.attr#dinifly_cacheStrategy}.
 * <p>
 * You can manually set the progress of the animation with {@link #setProgress(float)} or
 * {@link R.attr#dinifly_progress}
 */
class LottieAnimationView extends ImageView {
  ...
  private static final Map<String, WeakReference<LottieComposition>> weakRefCache =
      new HashMap<String, WeakReference<LottieComposition>>();
  ...
  public Rect getCompRect() { return composition.getBounds(); }
  ...
  /**
   * lottie will centerinside the drawable, if you want fixy to different mobile screen, use this function
   * note:!!!!! make sure the compostion layer has not matte layer
   */
  ...
  private void init(@Nullable AttributeSet attrs) {
    TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.DiniFlyAnimationView);
    defaultCacheStrategy = ta.getInt(
        R.styleable.DiniFlyAnimationView_dinifly_cacheStrategy,
        CacheStrategy.None);
    String fileName = ta.getString(R.styleable.DiniFlyAnimationView_dinifly_fileName);
    if (!isInEditMode() && fileName != null) {
      setAnimation(fileName);
    }
    if (ta.getBoolean(R.styleable.DiniFlyAnimationView_dinifly_autoPlay, false)) {
      lottieDrawable.playAnimation();
      autoPlay = true;
    }
    lottieDrawable.loop(ta.getBoolean(R.styleable.DiniFlyAnimationView_dinifly_loop, false));
    setImageAssetsFolder(ta.getString(R.styleable.DiniFlyAnimationView_dinifly_imageAssetsFolder));
    setProgress(ta.getFloat(R.styleable.DiniFlyAnimationView_dinifly_progress, 0));
    enableMergePathsForKitKatAndAbove(ta.getBoolean(
        R.styleable.DiniFlyAnimationView_dinifly_enableMergePathsForKitKatAndAbove, false));
    if (ta.hasValue(R.styleable.DiniFlyAnimationView_dinifly_colorFilter)) {
      addColorFilter(new SimpleColorFilter(ta.getColor(
          R.styleable.DiniFlyAnimationView_dinifly_colorFilter, Color.TRANSPARENT)));
    }
    if (ta.hasValue(R.styleable.DiniFlyAnimationView_dinifly_scale)) {
      lottieDrawable.setScale(ta.getFloat(R.styleable.DiniFlyAnimationView_dinifly_scale, 1f),
              ta.getFloat(R.styleable.DiniFlyAnimationView_dinifly_scale, 1f));
    }
   ...
   */
  @SuppressWarnings("WeakerAccess") public void setAnimation(final String animationName, final int cacheStrategy) {
   ...
   * using {@link R.attr#dinifly_cacheStrategy}.
   */
  public void setComposition(@NonNull LottieComposition composition) {
    ...
    if (compWidth > screenWidth ||
      ...
      if (fitFullscreenXY && !hasMatte()) {
        setScaleXY(xScale,yScale);
      } else {
        float maxScaleForScreen = Math.min(xScale, yScale);
        float centerInSide = Math.min(maxScaleForScreen, lottieDrawable.getScale());
        setScaleXY(centerInSide,centerInSide);
  ...
  public void removeAnimatorListener(Animator.AnimatorListener listener) {
    ...
    public void removeAllAnimatorListener() {
        lottieDrawable.removeAllAnimatorListener();
    }
  ...
  public void setScaleXY(float scaleX, float scaleY) {
    lottieDrawable.setScale(scaleX, scaleY);
    if (getDrawable() == lottieDrawable) {
      setImageDrawable(null);
      setImageDrawable(lottieDrawable);
    }
